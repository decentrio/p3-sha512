...
impl Encoder {
    ...
    pub fn flag_with_expr<AB: InteractionBuilder>(
        &self,
        vars: &[AB::Var],
        flag_idx_expr: &[(usize, impl Into<AB::Expr> + Clone)],
    ) -> AB::Expr {
        flag_idx_expr
            .iter()
            .fold(AB::Expr::ZERO, |acc, (flag_idx, expr)| {
                acc + self.get_flag_expr::<AB>(*flag_idx, vars) * (*expr).clone().into()
            })
    }
}
